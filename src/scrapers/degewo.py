"""
Degewo Scraper (Optimized for Live Updates).

This module defines the DegewoScraper class, optimized for detecting
...
- Uses efficient API-based approach via wohnraumkarte.de
...
class DegewoScraper(BaseScraper):
    """
    Handles fetching and parsing of apartment listings from degewo.de.

    Uses the wohnraumkarte.de API with dataSet=degewo to fetch listings.
    ...
    def __init__(self, name: str):
        """
        Initializes the Degewo scraper.
        ...
        self.base_url = "https://www.degewo.de"
        self.headers.update(
            {
    ...
    def get_current_listings(
        ...
        try:
            ...
            logger.debug(f"Fetched {len(batch_listings)} listings from Degewo API.")
            ...
            for listing_data in batch_listings:
                ...
                if identifier and identifier in known_ids:
                    logger.debug(
                        f"Hit known listing '{identifier}', stopping (newest-first)"
                    )
                    break
            ...
            if new_count > 0:
                logger.info(f"Found {new_count} new listing(s) on degewo.de")
            else:
                logger.debug("No new listings found on degewo.de")

        except requests.exceptions.RequestException as exc:
            logger.error(f"Request error for {self.api_url}: {exc}")
            raise
    ...
    def _extract_identifier_fast(self, listing_data: dict) -> Optional[str]:
        ...
        if slug:
            return f"{self.base_url}/immosuche/details/{slug}"
        return None
